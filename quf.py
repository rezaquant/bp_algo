...
from quimb.experimental import cluster_update
...
def req_backend(chi=None, 
                target_size = 2**34, alpha=64,
                progbar=False, optlib="cmaes", max_time="rate:1e8",
                max_repeats=2**8, subtree_size=12, parallel=True,
               ):
    ...
    opt = ctg.ReusableHyperOptimizer(
        ...
        minimize=f'combo-{alpha}',                  # target both flops and mops
        slicing_opts={'target_size': target_size},         # first do basic slicing
        ...
        parallel=parallel,  # optimize in parallel
        optlib=optlib,  # an efficient parallel meta-optimizer
        max_time=max_time,  # don't spend time optimizing cheap contractions
...
def path_tag(tn, taga, tagb, site_tags, max_distance=1, fillin=1):
    from quimb.tensor import networking
    tida, tidb = (tn.tag_map[taga], tn.tag_map[tagb])
    net = networking.get_path_between_tids(tn, *tida, *tidb)
    tids = tuple(net.tids)
    path_tags = []
    for tid in tids:
        tag = [ tag for tag in tn.tensor_map[tid].tags if tag in site_tags]
        path_tags.append(tag[0])

    tn_ = tn.select_local(path_tags, which='any', max_distance=max_distance, fillin=fillin, virtual=True)
    local_tags = [tag for tag in tn_.tags if tag in site_tags] 
    
    return tn_, (path_tags, local_tags)


def remap_to_peps(peps, sitemap_, Lx, Ly):
    peps = peps.copy()
    # retag peps to 2d coord:
    peps.retag_({ f"I{a}":f"I{b},{c}" for a, (b, c) in sitemap_.items()})
    site_tags = [ f"I{b},{c}" for a, (b, c) in sitemap_.items()]
    peps.reindex_({ f"k{a}":f"k{b},{c}" for a, (b, c) in sitemap_.items()})
    for a, (b, c) in sitemap_.items():
        peps[f"I{b},{c}"].add_tag(f"X{b}")
        peps[f"I{b},{c}"].add_tag(f"Y{c}")
    peps.view_as_(qtn.tensor_2d.PEPS, Lx=Lx, Ly=Ly, site_tag_id='I{},{}', x_tag_id='X{}', y_tag_id='Y{}',
                  site_ind_id='k{},{}',
                )
    return peps
...
def local_expectation_cluster(tn, cor, site_tags, max_distance=1, fillin=1, target_size=2**30, opt=None,
                              gauges = {}, tree_gauge_distance=4, peak_max=36,cutoff=1.e-12,
                              chi=40, to_backend=None, str_="Z", progbar=True, f_max=16, contract_="exact",
                             ):

    
    res_cor = bp_info_rho(cor)
    
    if len(cor) == 1:
        obs = qu.pauli(str_)
        obs_tensor = qtn.Tensor(obs.reshape(2,2), inds=res_cor["inds_rho"], tags="zz")
    
    if len(cor) == 2:
        obs = qu.pauli(str_) & qu.pauli(str_) 
    ...
    if len(res_cor["reg_tags"])==1:    
        taga, tagb = (res_cor["reg_tags"][0], res_cor["reg_tags"][0])
    else:
        taga, tagb = (res_cor["reg_tags"][0], res_cor["reg_tags"][1])
        
    
    tn_rho, (path_tags, rho_tags) = vidal_rho(tn, taga, tagb, site_tags, gauges=gauges, 
                                                max_distance=max_distance, fillin=fillin
                                             )

    res = req_backend(chi=chi, progbar=progbar, max_repeats=2**8, target_size=target_size)
    copt = res["copt"]
    
    if contract_=="appr":
        rho, (flops, peak)= apply_hyperoptimized_compressed(tn_rho, copt, chi, 
                                                        output_inds=tn_rho.outer_inds(), 
                                                        tree_gauge_distance=tree_gauge_distance, progbar=progbar, 
                                                        cutoff=cutoff, equalize_norms=True, 
                                                        f_max=f_max, peak_max=peak_max,
                                                        contract_=contract_,
                                                       )
        norm_ = complex(rho.trace(res_cor['leftinds_rho'], res_cor['rightinds_rho']))
        z = (rho & obs_tensor).contract(all)    
        val_ = complex(z / norm_)

    elif contract_=="exact":
        rho = tn_rho.contract(output_inds=tn_rho.outer_inds(), optimize=opt)
...
def local_expectation_bp(bp, tn, cor, site_tags, max_distance=1, fillin=1, opt=None,
                              gauges = {}, tree_gauge_distance=4, peak_max=36,cutoff=1.e-12,
                              chi=40, to_backend=None, str_="Z", progbar=True, f_max=16, contract_="exact",
                             ):
    ...
    if taga != tagb:
        tn_, (path_tags, local_tags) = path_tag(tn, taga, tagb, site_tags, max_distance=max_distance, fillin=fillin)
    else:
        tn_ = tn.select_local(taga, which='any', max_distance=max_distance, fillin=fillin, virtual=True)
        local_tags = [tag for tag in tn_.tags if tag in site_tags] 
        path_tags, local_tags = ([], local_tags)
    
    
    site_t = [(int(x.split(',')[0][1:]), int(x.split(',')[1])) for x in local_tags]
    
    inds_b = { f"b{i},{j}":f"k{i},{j}" for i in range(tn.Lx) for j in range(tn.Ly) }


    tn_rho = rho_bp(bp, site_t, inds_b, output_inds=res_cor["inds_rho"], contract = False, absorb_guage=True, fuse = True)
    
    # if to_backend:
    #     tn_rho.apply_to_arrays(to_backend)


    val_ = 0
    if contract_=="appr":
        res = req_backend(chi=chi, progbar=progbar, max_repeats=2**8)
        copt = res["copt"]
        rho, (flops, peak)= apply_hyperoptimized_compressed(tn_rho, copt, chi, 
                                                            output_inds=tn_rho.outer_inds(), 
                                                            tree_gauge_distance=tree_gauge_distance, progbar=progbar, 
        ...
        norm_ = rho.trace(res_cor['leftinds_rho'], res_cor['rightinds_rho'])
        z = (rho & obs_tensor).contract(all)
        val_ = complex(z / norm_)

    elif contract_=="exact":
        rho = tn_rho.contract(all, output_inds=tn_rho.outer_inds(), optimize=opt)    
    ...
    return val_, (tn_rho, local_tags)



def vidal_rho(tn, taga, tagb, site_tags, equalize_norms=False,
              max_distance=1, fillin=1, inplace=False, gauges={}):
    ...
    ta = tn[taga]
    tb = tn[tagb]
    inda = [ indx for indx in ta.inds if indx.startswith("k")]
    indb = [ indx for indx in tb.inds if indx.startswith("k")]
    inda, indb = (inda[0], indb[0])
    ...
    outer, inner = tn_.gauge_simple_insert(gauges)
    tnH = tn_.H
    tnH.reindex_({inda:inda.replace("k", "b"), indb:indb.replace("k", "b")})
    tn_rho = tnH & tn_
    return tn_rho, (path_tags, local_tags)
...
def floquet_circ(Lx, Ly, depth=4):
    ...
    if N == Lx*Ly:
        print("Qubits:", N, "edges:", len(edges))
    ...
    def gen_layer_gates(r, dt=0.2):
        for site in sites:
            yield qtn.Gate("RX", params=(dt*2,), qubits=(sitemap[site],), round=r)
        for sitea, siteb in edges:
            yield qtn.Gate("RZZ", params=(dt*2,), qubits=(sitemap[sitea], sitemap[siteb]), round=r)
        for site in sites:
            yield qtn.Gate("RX", params=(dt*2,), qubits=(sitemap[site],), round=r)
    ...
    for r in range(depth):
        gates.extend(gen_layer_gates(r))
    ...
    psi = circ.psi
    uni = circ.uni
    psi_ = psi.gate(qu.pauli("Z"), [0], inplace=False)
    tn = psi.H & psi_

    res = { "psi":psi, "tn":tn, "tn_norm": psi.H & psi, "circ":circ, "uni":uni   }
    return res

def vidal_to_bp(tn, site_tags, iter_=50, tol=1e-8, progbar=True, damping=0.02, opt="auto-hq", normalize=False):

    tn = tn.copy()

    gauges = {}
    tn.gauge_all_simple_(iter_, tol=tol, gauges=gauges, progbar=progbar, damping=damping, equalize_norms=True)
    normalize_gauges_l1(gauges)
    norm_0 = norm_l1(tn, gauges, site_tags, opt)
    print("norm_vidal ~", norm_0)

    
    bp = L1BP(tn, optimize=opt, site_tags=site_tags, damping=0.01)
    for key, message_ in bp.messages.items():
        inds = list(message_.inds)
        if len(inds)==1:
            g = gauges[inds[0]].copy()
            message_.modify(data=g)
        if len(inds)==2:
            g = gauges[inds[0]]
            g_ = gauges[inds[1]]
            g2 = g*g_
            g2 = ar.do("diag", g2)
            message_.modify(data=g2)
    ...
    est_norm = complex(mantissa * 10**(norm_exponent+tn.exponent))
    print("norm_bp ~", est_norm)
...
def floquet_tn(Lx, Ly, depth=4, cycle_peps=True, obs_="z2", backend_=True):
    from floquet.circuits import (trotter_2D_square, circ_gates, Z1_ti, Z2_ti, mpo_z2_, simulate)
    ...
    mpo = mpo_zz_center(x_dim*y_dim, "Z", where_=L//2)

    if obs_=="z2":
        #mpo = mpo_z2_(circ_info, "Z", form="left")
        mpo = mpo_zz_center(x_dim*y_dim, "Z", where_=L//2)

    if obs_=="z":
        mpo = mpo_z_select(x_dim*y_dim, [(Lx*Ly)//2], "Z")
    
    
    
    pepo_z = MPO_to_PEPO(mpo, x_dim, y_dim, cycle_peps=cycle_peps)
    pepo_z = MPO_to_PEPO_rotated(mpo, x_dim, y_dim, cycle_peps=cycle_peps)
    
    
    peps = peps_I(x_dim, y_dim, theta = 0, dtype = "complex128")
    if cycle_peps:
        peps = peps_cycle(peps, int(1))
    pepo_l_ = [pepo.copy() for _ in range(steps)]
    
    TN_peps, tags_list, pos_3d = gate_to_3dTN(peps, pepo_l_, pepo_z, steps, x_dim*y_dim, x_dim, y_dim, 1)
    TN_peps_norm, tags_list_, pos_3d_ = gate_to_3dTN_norm(peps, pepo_l_, pepo_z, steps, x_dim*y_dim, x_dim, y_dim, 1)
    #print(TN_peps.Lx, TN_peps.Ly, TN_peps.max_bond) 
    
    TN_peps_origin = TN_peps.copy()
    ...
    if backend_:     
        ...
        TN_peps.apply_to_arrays(backend_)
        pepo_z.apply_to_arrays(backend_)
        pepo.apply_to_arrays(backend_)
        peps.apply_to_arrays(backend_)

    
    res = {"tn3d":TN_peps, "fix3d":pos_3d, "xpepo":pepo_z, "pepo":pepo, "peps":peps, "z2_mpo":mpo}
    return res
...
def projector(tmi, tmj):
    ...
    #Pr_excited = ar.do('asarray', Pr_excited, like=backend)
    return Pr_excited



def projector_gauge(tmi, tmj, g):
    to_backend_ = ar.infer_backend(tmi)
    ...
    # Create the identity matrix of the same shape as the projector
    I = ar.do("diag", g)  
    I = ar.do('asarray', I, like=backend, dtype=dtype, device=device)
...
def draw_info(m_=1000):
    colors = ["magenta", "darkblue", "darkslateblue", "orange", "darkgreen", "gold", "violet", "darkred", "teal", "orchid", "pink"]
    colors += ["tan", "skyblue", "beige", "limegreen", "peru", "darkmagenta", "steelblue", "plum", "hotpink", "cyan", "darkviolet"]
    colors += ["brown", "bisque", "wheat", "darkgreen", "mintcream", "lightcyan", "teal", "skyblue", "indigo", "gray"]
    hatches = [
        
        '//', '\\\\', '||', '--', '++', 'xx', 'oo', 'OO', '..', '**',  # Denser patterns
        '/-', '\\-', '/|', '\\|', '+-', 'x-',  # Combinations of patterns
        'o-', '.-', '*-', 'O-',  # Circles, dots, stars with horizontal lines
        'ox', 'o.', 'O.', 'Ox',  # Combinations of circles, dots, and stars
        '//-', '\\\\-', '/\\', 'x|', 'x+',  # Mixed grid and diagonal patterns
        'o|', 'O|', '.|', '*|',  # Circles, stars, and dots with vertical lines
        '/', '\\', '|', '-', '+', 'x', 'o', 'O', '.', '*',  # Basic patterns
    ]
    shapes = ['s', 'D', 'p', '8',  '<', '>', '^', 'h', 'H']
    colors = colors*m_
    hatches = hatches*m_
    ...
    return colors, hatches, shapes


def dense_to_mps(tn, site_tags, opt="auto-hq", bnd = 16, max_distance=2, 
                   error_= 1.e-8, cutoff=0, iter_=10, inplace=False, print_=False):
    
    # copy tn input
    tn = tn if inplace else tn.copy()
    
    
    for tag in tqdm(site_tags):
    
        tid = tn._get_tids_from_tags(tag)
        ...
        if len(dims)>3:
            tdense = t.data
            reindex_ = { f"k{count}":i for count, i in enumerate(t.inds) }
            mps = qtn.MatrixProductState.from_dense(tdense, dims=dims, tags=tag, **{"cutoff":cutoff})
            mps.reindex_(reindex_)
            bnd_ = int(bnd*1.)
            mps_ = mps.copy()
            for _ in range(iter_):
                mps_ = mps.copy()
                mps_.compress("left", max_bond=bnd_, cutoff=cutoff)
                error = abs(1-complex(fidel_mps(mps_, t)).real)
                if error < error_:
                    break
                else:
                    bnd_ = bnd_+10
            if print_:
                print("bnd-mps", mps.max_bond(), "bnd-compress", mps_.max_bond(), 1-complex(fidel_mps(mps_, t)).real)
                mps_.canonize(mps_.L//2,)
                print(mps_.show())
            tn |= mps_
        else:
            tn |= t
    ...
    tn.full_simplify_(seq='R', output_inds=None, atol=1e-12, inplace=True)
    site_tags = []
    # for i, t in enumerate(tn):
    #     t.drop_tags(tags=[i for i in t.tags if i.startswith("R")])
    #     t.add_tag(f"R{i}")
    #     site_tags.append(f"R{i}")


    res = {"site_tags":site_tags, "tn":tn}
    return res


def edges_cluster(tn, map_id_tags, cut_edges, site_tags):
    


    # finds edges that connect the clusters:
    inds = set()  # Use a set directly to avoid duplicates from the start
    map_id_inds = {}
    for ids, tags in  map_id_tags.items():
        tn_ = tn.select(tags, which="any")
        inds.update(tn_.outer_inds())  # Add the outer indices to the set directly
        map_id_inds |= {ids: list(tn_.outer_inds())}
        
    inds = list(inds)  # Convert the set back to a list 
    
    edges = []
    map_edge_ids = {}
    for ix in inds:
        try:
            id0, id1 = tuple(tn.ind_map[ix])
        except:
            pass
        t0 = tn.tensor_map[id0]
        t1 = tn.tensor_map[id1]
        tag = tuple([ i for i in t0.tags if i in site_tags] + [ i for i in t1.tags if i in site_tags])
        tag = tuple(sorted(tag))
        edges.append(tag)
        cluster_ids = find_shared_keys(map_id_tags, tag)
        map_edge_ids |= {ix: cluster_ids}
    
    edges = list(set(edges))
    
    if len(edges) != cut_edges:
        print("not matching edges")

    res = {"edges": edges}
    return res


def inser_gauges_(tn, cluster_tags, bp):

    print("\033[34m Notice: only activate this in Vidal's gauges \033[0m")
    print("\033[32m put back gauges into internal bonds \033[0m")
    
    for tags in cluster_tags:
        tn_cluster = tn.select(tags, "any")
        for ix in tn_cluster.inner_inds():
                g = bp.gauges[ix]
                g = g**0.5
                tl, tr = tn_cluster._inds_get(ix)
...
def clustering(tn, site_tags, opt, n_parts=4, n_sites_=None, inplace=False, seed=12, print_=False):
    
    tn = tn if inplace else tn.copy()
    
    res_partition = partition_tn(tn, site_tags,  n_parts=n_parts, n_sites_=n_sites_, inplace=inplace, seed=seed)
    cluster_tags = res_partition["cluster_tags"]


    # contract clusters:
    for st in tqdm(cluster_tags):
    ...
    for i, t in enumerate(tn):
        #t.drop_tags()
        t.drop_tags(tags=[i for i in t.tags if i.startswith("R")])
        t.drop_tags(tags=[i for i in t.tags if i.startswith("temp")])
        t.drop_tags(tags=[i for i in t.tags if i.startswith("block")])
        t.add_tag(f"R{i}")
        site_tags.append(f"R{i}")

    if print_:
        print("max_bnd", tn.max_bond())
        print( "max/min: clusters:", max(res_partition["clusters_l"]), min(res_partition["clusters_l"]), "edges:", len(cluster_tags))

    res = {"tn":tn, "site_tags":site_tags }
    return res
...
def obe_measure(rho, chi_, output_inds, to_backend_, obs_ = qu.pauli("Z") & qu.pauli("Z"), progbar=False, f_max = 15 , peak_max=34 ):
    ...
    if chi_:
        res = req_backend(chi_, progbar=progbar, max_repeats=2**9)
        print(res["copt"])
        rho_appro, (f, peak_) = apply_hyperoptimized_compressed(rho, res["copt"], chi_, output_inds=output_inds,
...
def normalize_gauges_l1(gauges):
    for ix, g in gauges.items():
        norm_ = ar.do("linalg.norm", g)
        gauges[ix] = g / norm_

def normalize_gauges_l2(gauges):
    for ix, g in gauges.items():
        g2 = g*g
        norm_ = ar.do("linalg.norm", g2)
        gauges[ix] = g / norm_
    

def norm_l1(tn, gauges, site_tags, opt):

    lnorm_ = []
    for tag in tqdm(site_tags):
        tn_site = tn.select(tag, "any")
        tn_site_gauged = tn_site.copy()
        for ix in tn_site.outer_inds():
            g = gauges[ix]
            tn_site_gauged |= (qtn.Tensor(g, inds = [ix,], tags=["g"])/ar.do("linalg.norm", g))     
        
        lnorm = (tn_site_gauged).contract(all, optimize=opt)
        lnorm_.append(complex(lnorm))

    
    norm_l = [ np.log10(i) for i in  lnorm_] + [tn.exponent]
    return 10**sum(norm_l)

def guage_l2_insert(tn, gauges, site_tags):
    tn = tn.copy()
    for tag in tqdm(site_tags):
        tn_ = tn.select([tag], "all")
        tn_site = tn.select([tag]+["KET"], "all")
        tn_site_ = tn.select([tag]+["BRA"], "all")
        
        for ix in tn_.inner_inds():
            ix_ = qtn.rand_uuid()
            iy_ = qtn.rand_uuid()
            g = gauges[ix].data
            norm_ = do("linalg.norm", g)
            g = g / norm_
            g = qtn.Tensor(ar.do("diag", g), inds = [ix_,iy_], tags=["g",  tag])     
            tn_site.reindex_({ix:ix_})
            tn_site_.reindex_({ix:iy_})
            tn &= g    
            #print(tn_site, tn_site_, g)

def norm_l2(tn, gauges, site_tags,opt):
    tn = tn.copy()
    lnorm_ = []
    for tag in tqdm(site_tags):
        ...
        tn_site = tn.select([tag]+["KET"], "all")
        tn_site_ = tn.select([tag]+["BRA"], "all")
        tn_site = tn_site.copy()
        tn_site_ = tn_site_.copy()

        
        for ix in tn_site.outer_inds():
            g = gauges[ix]
            tn_site |= qtn.Tensor(g, inds = [ix,], tags=["g"])     
        lnorm = (tn_site).contract(all, optimize=opt)
        lnorm_.append(complex(lnorm))

        for ix in tn_site_.outer_inds():
            g = gauges[ix]
            tn_site_ |= qtn.Tensor(g, inds = [ix,], tags=["g"])     
        lnorm = (tn_site_).contract(all, optimize=opt)
        lnorm_.append(complex(lnorm))
...
# Accept only tn that do not have hyperindices:
def partition_tn(tn, site_tags, n_sites_=None, n_parts=4, seed=4, inplace=False, print_=False):

    res = {}
    tn = tn if inplace else tn.copy()
    
    if n_sites_:
        n_parts = len(tn.tensors) // n_sites_

    
    # Assign a linear tag register to each tensor:
    num_nodes = 0
    for i, t in enumerate(tn):
        t.add_tag(f"temp{i}")
        num_nodes += 1
    ...
    edges = tn.inner_inds()
    num_nets = len(edges)
    edge_weights = [ int(tn.ind_size(ix)) for ix in edges]
    res |= {"edges": num_nets}
    ...
    for edge in edges:
        tids = tn.ind_map.get(edge, [])
        
        # Check for dangling bonds
        if len(tids) != 2:
            print("Warning: dangling bond exists for edge:", edge)
            continue
        ...
        def extract_coordinate(tensor):
            tag = next((tag for tag in tensor.tags if tag.startswith("temp")), None)
            if not tag:
                raise ValueError(f"No 'temp' tag found in tensor: {tensor}")
            match = re.search(r'temp(\d+)', tag)
            if not match:
                raise ValueError(f"Invalid 'temp' tag format in tag: {tag}")
            return int(match.group(1))
        
        try:
            cor1d = extract_coordinate(t1)
            cor2d = extract_coordinate(t2)
        except ValueError as e:
            print(f"Error processing edge {edge}: {e}")
            continue
        
        # Update edges_map
        edges_map[edge] = (cor1d, cor2d)

    
    edges_1d = [ i for i in list(edges_map.values())]
    hyperedges = [node for edge in edges_1d for node in edge]
    hyperedge_indices = list(range(0, len(hyperedges) + 1, 2))
    # print("hyperedges =", hyperedges)
    # print("hyperedge_indices =", hyperedge_indices)
    # print(num_nodes, num_nets)    

    #edge_weights += [edge_weights[-1]] 
    node_weights = [1 for i in range(num_nodes)]

    if print_:
        print("tns", len(tn.tensors), "partions", n_parts)
    
    hypergraph = kahypar.Hypergraph(num_nodes, num_nets, hyperedge_indices, hyperedges, n_parts, edge_weights, node_weights)
    context = kahypar.Context()
    context.loadINIconfiguration("km1_kKaHyPar_sea20.ini")
    
    context.setK(n_parts)
    context.setSeed(seed)
    context.setEpsilon(0.03)
    # Suppress internal output
    context.suppressOutput(True)  # This disables all internal output from KaHyPar
    ...
    for i in range(len(hyperedge_indices) - 1):
        # Get the nodes in the current hyperedge
        start = hyperedge_indices[i]
        end = hyperedge_indices[i + 1]
        nodes_in_edge = hyperedges[start:end]
    
        # Find the unique blocks for these nodes
        blocks = {hypergraph.blockID(node) for node in nodes_in_edge}

        # If nodes are in more than one block, it's a cut edge
        if len(blocks) ==2:
            cut_edges.append(tuple(blocks))
            cut_edges_ += 1
            
    
    res |= {"block_edges":cut_edges}
    res |= {"cut_edges":cut_edges_}
    ...
    for node in hypergraph.nodes():
        block_id = hypergraph.blockID(node)
        block_map |= {block_id:[]}
    ...
    for node in hypergraph.nodes():
        block_id = hypergraph.blockID(node)
        tag = [tag for tag in tn.select(f"temp{node}").tags if tag in site_tags]
        #print(f"Node {node} is assigned to block {block_id}", tag[0])
        block_map[block_id].append(tag[0])
    
    blocks = list(block_map.keys()) 
    clusters = list(block_map.values())
    clusters_lengths = [len(i) for i in clusters]
    #print(clusters_lengths)
    
    clusters_ = list(itertools.chain.from_iterable(clusters))
    
    if set(clusters_) != set(site_tags):
        print("clusters_ != site_tags")

    block_tags = []
    for key, value in block_map.items():
        for tag in value:
            block_tags.append(f"blockid{key}")
            tn[tag].add_tag(f"blockid{key}")
    block_tags = list(set(block_tags))



    
    
    # find unique index of dges connecting clusters to each other
    inds_cluster = []
    for cluster_tags in clusters:
        tn_ = tn.select(cluster_tags, which="any")
        for indx in tn_.outer_inds():
            inds_cluster.append(indx)
    inds_cluster = list(set(inds_cluster))


    res_ = edges_cluster(tn, block_map, cut_edges_, site_tags)
    edges = res_["edges"]
    #print("edges", edges, len(edges))

    
    if print_:
        print("cluster-size:", max(clusters_lengths), min(clusters_lengths))
    ...
    res |= {"map_id_tags":block_map, "edges_indx": inds_cluster , "cluster_tags":clusters, "clusters_l":clusters_lengths}
    res |= {"block_tags":block_tags, "edges":edges}
    return res




# def cluster_neighbors_r(cluster_edges, r):
#     """
#     Compute neighbors of nodes at a specific distance r.

#     Parameters:
#         cluster_edges (list of tuples): List of edges representing the graph.
#         r (int): The distance to compute neighbors for.

#     Returns:
#         dict: Dictionary where keys are nodes, and values are lists of neighbors at distance r.
#     """
#     # Create a dictionary to store the graph structure
#     graph = {}
...
#     # Function to compute neighbors at distance r
#     def neighbors_at_distance_r(graph, r):
#         result = {node: set() for node in graph}

#         # Start with immediate neighbors (distance 1)
#         current_level = {node: graph[node] for node in graph}
#         visited = {node: graph[node].copy() for node in graph}  # Track visited nodes

#         for distance in range(2, r + 1):
#             next_level = {node: set() for node in graph}
#             for node in current_level:
#                 for neighbor in current_level[node]:
#                     next_level[node].update(graph[neighbor])
#             # Remove already visited nodes
#             for node in next_level:
#                 next_level[node].difference_update(visited[node])
#                 visited[node].update(next_level[node])
#                 next_level[node].discard(node)  # Avoid self-loops
#             # Update the current level for the next iteration
#             current_level = next_level

#         return {node: list(current_level[node]) for node in current_level}

#     return neighbors_at_distance_r(graph, r)

def eleminate_(data):
    def canonicalize_tuple(t):
        return tuple(sorted(map(tuple, t)))
        # Remove duplicates while preserving order, keep empty lists
    
    unique_data_no_empty_duplicates = []
    seen = set()
    
    for t in data:
        if t:  # For non-empty tuples
            canon_tuple = canonicalize_tuple(t)
            if canon_tuple not in seen:
                seen.add(canon_tuple)
                unique_data_no_empty_duplicates.append(t)
        elif not any(x == [] for x in unique_data_no_empty_duplicates):  # Keep only one empty list
            unique_data_no_empty_duplicates.append(t)
    
    
    return unique_data_no_empty_duplicates  
    

def local_configs_(tn, edges, site_tags, max_distance=1, fillin = 0, r_ = 4, 
                  loopy_filter = True,
                  print_=True, draw_=True):
    
    pair_excited_l = []
    
    for str_ in site_tags:
        tn_local = tn.select_local(str_, which='all', max_distance=max_distance, 
                                   fillin=fillin, virtual=True, include=None, exclude=None,
                                  )
        ...
        if draw_:
            tn.draw(str_, #block_tags
                    node_hatch={str_:'////'}, node_shape={str_:"H"},
                    legend=False, node_scale=1., highlight_inds=inds, show_tags=False, figsize=(8,8))
        ...
        for ix in inds:
            ...
            t0 = tn.tensor_map[id0]
            t1 = tn.tensor_map[id1]
            tag_ = tuple([ i for i in t0.tags if i in site_tags] + [ i for i in t1.tags if i in site_tags])
            tag_ = tuple(sorted(tag_))
            edges_local.append(tag_)
    
        if print_:
            print(str_, f"edges={len(inds)}", len(edges_local))
        
        if r_ > len(edges_local):
            print(f"warning: number of edges are smaller than given {r_} combinations: {len(edges_local)}")
            r_ = len(edges_local) 
        
        
        for r in range(0, r_ + 1 ):
            pair_excited_ = combine_elements(edges_local, r, sites = [], loopy_filter=loopy_filter)
            #pair_excited_ = list(itertools.combinations(edges_local, r))
            pair_excited_l += pair_excited_


        pair_excited_l = eleminate_(pair_excited_l)
        
    
    return pair_excited_l
...
def local_configs(tn, res_partition, site_tags, max_distance=1, fillin = 0, r_ = 4, print_=True, draw_=True):
    
    edges_indx = res_partition["edges_indx"]
    map_id_tags = res_partition["map_id_tags"]
    edges = res_partition["edges"]

    pair_excited_l = []
    pair_gs_l = []
    
    for i in range(len(map_id_tags.keys())):
        tn_local = tn.select_local(f"blockid{i}", which='all', max_distance=max_distance, 
                                   fillin=fillin, virtual=True, include=None, exclude=None,
                                  )
    
        inds = tn_local.inner_inds()
        inds = [ i for i in edges_indx if i in inds ]
        
        if draw_:
            tn.draw(f"blockid{i}", #block_tags
                    node_hatch={f"blockid{i}":'xx'}, node_shape={f"blockid{i}":"H"},
                    legend=False, node_scale=1., highlight_inds=inds, show_tags=False, figsize=(8,8))
        ...
        if print_:
            print(f"block{i}", f"edges={len(inds)}", len(edges_local))
        
        if r_ > len(edges_local):
            print(f"warning: number of edges are smaller than given {r_} combinations: {len(edges_local)}")
            r_ = len(edges_local) + 1
        
        
        for r in range(0, r_ ):
            pair_excited_ = list(itertools.combinations(edges_local, r))
            pair_excited_ = [ tuple(sorted(i))  for i in pair_excited_]
            pair_excited_l += pair_excited_
        
        pair_gs_l = []
        for i_ in pair_excited_l:
            pair_gs_l += [list(set(edges) - set(list(i_)))]
        
    
    
    
    # eliminate the pairs that are common 
    pair_gs_l, pair_excited_l = eliminate_(pair_gs_l, pair_excited_l)
    
    return pair_gs_l, pair_excited_l
    

    
def configs_(edges, r_=3):
    
    pair_excited_l = []
    for r in range(0, r_ ):
        pair_excited_ = list(itertools.combinations(edges, r))
        pair_excited_ = [ tuple(sorted(i))  for i in pair_excited_]
        pair_excited_l += pair_excited_
...
def cluster_neighbors_r(cluster_edges, r):
    """
    Compute neighbors of nodes at a specific distance r using networkx.
    ...
    graph = nx.Graph()
    graph.add_edges_from(cluster_edges)

    # Compute neighbors at distance r
    neighbors_at_r = {}
    for node in graph.nodes:
        # Use the networkx function to get nodes within a radius
        all_within_r = nx.single_source_shortest_path_length(graph, node, cutoff=r)
        
        # Filter out nodes at exactly distance r
        neighbors_at_r[node] = [n for n, dist in all_within_r.items()]

    return neighbors_at_r
...
def fix_plot(Lx=2, Ly=2, Lz=None, ):
...
def find_shared_keys(data_map, tags):
    result = []
    for tag in tags:
        keys_containing_tag = [key for key, values in data_map.items() if tag in values]
        if keys_containing_tag: 
            result.append(keys_containing_tag[0])
    return tuple(result)
...
def filter_unique_sublists(nested_list):
    unique_sublists = []
    for sublist in nested_list:
        if not any(has_common_elements(sublist, unique) for unique in unique_sublists):
            unique_sublists.append(sublist)
    return unique_sublists


def run_bp(tn, site_tags, normalize=True, progbar=False, 
           max_iterations=520, tol=1.e-7, 
    ...
    if print_:
        print(f"bp runs with the {update} update ~ iter = {max_iterations_}")

    
    bp = L1BP(tn, optimize=opt, site_tags=site_tags, damping=damping, normalize=normalize_)
    bp.update=update
    ...
    if normalize:
        ...
        tn_normalize_wrt_massages(tn, bp, site_tags)
...
def filter_loopy_combin(combs_2, sites):
    comb_two_f = []
    for combs_2_ in combs_2:
        ...
        if 1 in values_list:
            ...
            # if sites exist, then we are interested in the density matrix: dangling site alone does not result in zero;
            if sites:
                # if dangling sites are sitting on the density matrix positions, then they are valid as loops can be non-zero
...
def combine_elements(edges, length, sites = [], loopy_filter=True):
    
    comb_two_f = list(itertools.combinations(edges, length))
    
    #filter non-loops edges:
    if loopy_filter:
        comb_two_f = filter_loopy_combin(comb_two_f, sites)
...
def gen_loop(tn, loop_length=4, intersect=True, region_tag=[], site_tags=[], str_="R"):
    ...
    for g in gen:
        ...
        if len(g.inds) == loop_length:
            ...
            for t_pair in tids_pair:
                ...
                pattern = re.compile(rf"{str_}-?\d+(\.\d+)?(,-?\d+(\.\d+)?)*")
...
def loss_peps(peps, peps_fix, opt, cost_f, val_, chi_bmps= 60, mode = "appr", ):
...
def gate_su(
    tn, gauges, where, U, bnd=None, max_distance=1, fillin=1, steps=10,
    method="autodiff", smudge=1e-6, opt="auto-hq", progbar=False,
    tol=1.e-9, fidel_=False, draw_=False, renorm=False,
):
    """Apply a two/one-site gate to a tensor network with local optimization."""

    if len(where) == 1:
        inda, = tn.gen_inds_from_coos(where)
        qtn.tensor_network_gate_inds(tn, U, [inda], contract=True, inplace=True)
        return

    print("back")
    taga, tagb = tn.gen_tags_from_coos(where)
    inda, indb = tn.gen_inds_from_coos(where)
    ta, tb = tn[taga], tn[tagb]
    indx = [i for i in ta.inds if i in tb.inds][0]
    dtype = ar.get_dtype_name(ta.data)

    if not bnd:
        bnd = ta.shared_bond_size(tb)

    psi_local = tn.select_local(
        (taga, tagb), "any",
    ...
    outer, inner = psi_local.gauge_simple_insert(gauges, smudge=smudge)

    if draw_:
        print(psi_local.draw((taga, tagb), show_inds="bond-size", legend=False, show_tags=False))

    psi_local.balance_bonds_()
    
    if renorm:
        #psi_local.equalize_norms_()
        print("renorm")
        norm_ = (psi_local.H & psi_local).contract(all, optimize=opt)
        psi_local *= (norm_**-0.5)

        
    if fidel_:
        psi_localg = qtn.tensor_network_gate_inds(
            psi_local, U, [inda, indb], contract=False, inplace=False
        )

    print("Hi")
    cluster_update.gate_inds_nn_fit(
        psi_local,
        U,
        inda,
        indb,
        ...
        pregauge=2,
        steps=steps,
        contract_opts={"optimize": opt},
        fit_opts={"progbar": progbar, "tol": tol},
    )
    print("Hi_1")
    ta, tb = psi_local[taga], psi_local[tagb]
    bnd = ta.shared_bond_size(tb)

    if fidel_:
        print("bnd", bnd, "fidel", fidel_mps(psi_localg, psi_local))

    ones_vec = ar.do("ones", (bnd,), dtype=dtype)
    for i, (t, ix, g) in enumerate(inner):
        if ix == indx:
            inner[i] = (t, ix, ones_vec)

    gauges[indx] = ones_vec

    psi_local.gauge_simple_remove(outer=outer, inner=inner)
    psi_local.gauge_local_(
        (taga, tagb),
        "any",
        max_distance=1,
        max_iterations=6,
...
def gen_loop_(tn, loop_length=4, region_tag=[], site_tags=[], str_="R"):
...
def env_rho(info_pass, tree_gauge_distance=4, f_max = 14 , peak_max=34, prgbar=True, draw_=False, opt="auto-hq"):
...
def  mpo_2d_His_(L_x,L_y,L_L, data_type="float64"):

    Lx = L_x
    Ly = L_y
    L = L_L
    dic_ = {  (i,j): i*Ly + j for i in range(L_x) for j in range(Ly) }
...
def peps_norm(peps, opt, chi=20, 
    ...
    return complex(peps_norm)


def peps_normalize(peps, opt, chi=20, 
            layer_tags=['KET', 'BRA'], 
            equalize_norms=False, 
            max_separation=1, 
            sequence = ['xmin', 'xmax', 'ymin', 'ymax'],
...
def peps_fitting_coulmn(peps, peps_, Lx, Ly, chi, opt, x_range,y_range, opt_distance, 
                        ...
                        progbar = False, sequence=None, norm_fix = 1, 
                        ):
...
def peps_fitting_row(peps, peps_, Lx, Ly, chi, opt, x_range, y_range, opt_distance, iterate = 50, 
                    ...
                    progbar = True, sequence=None,norm_fix = 1,
                    ):
    ...
    for _ in tqdm(range(iter_)):
        if min(y_range) != max(y_range):
            peps = peps_fitting_coulmn(peps, peps_fix, Lx, Ly, chi, opt, x_range, y_range, opt_distance, iterate = iterate, sequence = sequence,
                                            cost_f = cost_f, optimizer = "L-BFGS-B", normalize=normalize, progbar=progbar)
...
def apply_2dtn(peps, G, where, bond_dim, 
    ...
    if len(where) == 1:
        ...
        if bra:
            GH = G.H
            qtn.tensor_network_gate_inds(peps, GH, [f"b{i},{j}"], contract=True, 
                                    tags=tags, info=None, inplace=True,
                                    **{"cutoff":cutoff}
                                    )    
    # two-qubit gate:
...
def rho_bp_excited_loops_(tn, bp, pass_rho, edges_f, obs_tensor, opt="auto-hq", draw_=False, pari_exclusive=None):
    ...
    res_rho = env_rho(pass_rho, draw_= draw_, opt=opt)
    
    #rho_data = res_rho["rho_data"]
    #flops_l = res_rho["flops_l"]
    if res_rho:
        rho_l = res_rho["rho_l"]
        obs_real_l = [ (rho | obs_tensor).contract(all, optimize=opt) for rho in rho_l]
        obs_ = sum(obs_real_l)


        
        norm_loop_real = [ rho.trace(pass_rho['leftinds_rho'], pass_rho['rightinds_rho'])  for rho in rho_l]
        norm_loop = sum(norm_loop_real)
...
def loop_gen_local(bp, circum = None, tags_cluster=[], sites=[], circum_g = None, tn_flat=None, 
                   site_tags=[], intersect=True,
    ...
    if circum_g:
        ...
        res = gen_loop(tn_flat, loop_length=circum_g, intersect=True, site_tags=site_tags, str_=str_)
        if  res["loop_pair"]:
            loops_global = res["loop_pair"]
        res = gen_loop(tn_flat, loop_length=circum_g, intersect=False, site_tags=site_tags, str_=str_)
        if  res["loop_pair"]:
            loops_global += res["loop_pair"]
        res = gen_loop_(tn_flat, loop_length=circum_g, site_tags=site_tags, str_=str_)
        if  res["loop_pair"]:
            loops_global += res["loop_pair"]
...
def eliminate_(pair_gs_l, pair_excited_l):
...
def bp_excited_loops_(tn_, bp_, loop_l, progbar=False, chi=None, max_repeats_=2**6, parallel=True, 
                     contract_=True, pari_exclusive=[], tree_gauge_distance=4, f_max = 15 , peak_max=34,
                      prgbar=False, opt="auto-hq", inplace=False,
                    ):
...
def bp_slices(bp, pair_excited=[]):

    tn_tags = list(set(itertools.chain(*pair_excited)))
    tn_local = qtn.TensorNetwork([bp.local_tn[str_] for str_ in tn_tags])


    messages_excited = []
    for pair_ in pair_excited:
        
        i, j = pair_
        pr_excited_tn = bp.projects[pair_]
        ...
        map_inds = { left_inds_[count]:right_inds_[count]      for count in range(len(right_inds_)) }
        tn_j.reindex_(map_inds)
        messages_excited.append(pr_excited_tn)
    
    
    #messages = [ (k, str_) for str_ in tn_tags for k in bp.neighbors[str_] if tuple(sorted((str_, k))) not in pair_excited]
    messages_gs = []
    for str_ in tn_tags:
        for k in bp.neighbors[str_]:
            if tuple(sorted((str_, k))) not in pair_excited:
                mij = bp.messages[(k, str_)].copy()
                inds_ij = list(mij.inds)
                map_ = { i:qtn.rand_uuid() for i in inds_ij }
                mij.reindex_(map_)
                tn_local[str_].reindex_(map_)
                messages_gs.append(mij)

    

    tn_local |= qtn.TensorNetwork(messages_excited+messages_gs, virtual=True)
    return tn_local
...
def prod_pepso(x_dim = 3, y_dim = 3, dt_=0.2, theta=0,
               cycle_peps = True, step_=0, bnd = 4, chi_bmps= 40, dtype="complex64",
              ):
    ...
    y_dim = y_dim
    steps = 16
    theta, J, h, hz, dt = theta, 1, 2, 0.0, dt_
    order = 'second'
    obs = {'Z': Z1_ti(x_dim,y_dim), 'Z2': Z2_ti(x_dim,y_dim)}
    circ_info = {"x_dim":x_dim, "y_dim":y_dim, "theta":theta, "J":J, "h":h, "dt":dt, "hz":hz, "order":order}
    ...
    peps = peps_I(Lx, Ly, theta = theta, dtype = dtype)
    if cycle_peps:
        peps = peps_cycle(peps, int(1))
...
def rho_bp(bp, site_t, inds_b, output_inds=["k2,2", "k2,3", "b2,2", "b2,3"], contract = True, absorb_guage=False, fuse = True):
    ...
    #rho, inds = bp.partial_trace(site_t, optimize=opt)
    rho, inds = partial_trace(bp, site_t, optimize=opt)
...
def tn_normalize_wrt_massages(tn, bp , site_tags):
    bp.normalize_messages()
    
    res = bp_excited_loops(tn, bp, [[]], chi=10, contract_=False, prgbar=False)
    tn_l = res["tn_l"]
    tags = list(site_tags)
    tn_ = tn_l[0]
    norm_l = []
...
def apply_hyperoptimized_compressed(tn, copt, max_bond, output_inds=None, tree_gauge_distance=4, progbar=False, cutoff=1.e-12, equalize_norms=True, f_max = 14 , peak_max=33.5, contract_ = True):
    ...
    if flops > 1.e-8:
        if np.log10(flops) < f_max and peak < peak_max:
            if contract_:
                ...
                return None, (flops, peak)
                
        else:
            print("warning: over-resources", np.log10(flops), peak, "ref:", (f_max, peak_max))
            return None, (flops, peak)
...
def pepo_flat_hypercompress(x_pepo, peps, chi, copt, 
    ...
    res, info = apply_hyperoptimized_compressed(tn, copt, chi, output_inds=None, 
                                            tree_gauge_distance=tree_gauge_distance, 
                                            progbar=progbar, f_max=f_max, equalize_norms=equalize_norms,
                                            cutoff=1.e-12,
                                         )
    
    
    main, exp=(res.contract(), res.exponent)
    return main * 10**(exp), info
...
def gate_pepo_(gate_, where_, Lx, Ly, cycle, cutoff=1.e-13, 
                ...
                dtype="complex128", bnd=32,
                sequence = ('av', 'bh', "ah", "bv"),contract='split',
                trans=True, max_iterations=450, tol=1.e-9,
                ):
